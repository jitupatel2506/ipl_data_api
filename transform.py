...
VALID_CATEGORIES = ["cricket", "kabaddi", "football"]
...
def load_sonyliv_matches():
    ...
    if isinstance(data, dict):
        for m in data.get("matches", []):
            ...
            if not m.get("isLive", False) or category not in VALID_CATEGORIES:
                continue
            matches.append({
                "channelNumber": int(m.get("contentId", 0)) or 900,
                "platform": "SonyLiv",
                "linkType": "app",
                "channelName": m.get("event_name") or "Unknown Match",
...
def pick_stream_url(m):
    candidates = [
        m.get("adfree_url"), m.get("adfree_stream"),
        m.get("dai_url"), m.get("daiUrl"),
        m.get("stream_url"), m.get("video_url")
    ]
...
def main():
    ...
    for m in fan_matches:
        ...
        category = (m.get("category") or m.get("event_category") or "").lower()
        if "live" not in status or category not in VALID_CATEGORIES:
            continue
